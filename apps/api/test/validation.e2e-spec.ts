...
describe('Validation E2E Tests', () => {
  ...
  const mockJobsService = {
    ...
    getJobById: jest.fn().mockResolvedValue({ id: 'test-job-1', status: 'completed' }),
    getJobStats: jest.fn().mockResolvedValue({ completed: 10, running: 2 }),
    getBullBoardInfo: jest.fn().mockResolvedValue({ url: 'http://localhost:3000/queues' }),
  };
  ...
  const mockPraPipelineService = {
    ...
    triggerFullPipelineRefresh: jest.fn().mockResolvedValue({ jobId: 'pra-job-2' }),
  };

  const mockHospitalsService = {
    getHospitals: jest.fn().mockResolvedValue({ hospitals: [], total: 0 }),
    getHospitalById: jest.fn().mockResolvedValue({ id: 'hospital-1', name: 'Test Hospital' }),
    getHospitalPrices: jest.fn().mockResolvedValue({ hospitalId: 'hospital-1', prices: [] }),
  };

  const mockPricesService = {
    getPrices: jest.fn().mockResolvedValue({ prices: [], total: 0 }),
    comparePrices: jest.fn().mockResolvedValue({ service: 'MRI', comparisons: [] }),
    getPricingAnalytics: jest.fn().mockResolvedValue({ analytics: {} }),
    getPriceById: jest.fn().mockResolvedValue({ id: 'price-1', service: 'MRI' }),
  };
  ...
  const mockAnalyticsService = {
    ...
    exportData: jest.fn().mockResolvedValue({ downloadUrl: 'http://example.com/export.csv' }),
  };
  ...
  beforeEach(async () => {
    const moduleFixture: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        { provide: HospitalMonitorService, useValue: mockHospitalMonitorService },
