...
describe("Authentication & Authorization (e2e)", () => {
  ...
  beforeAll(async () => {
    ...
    try {
      const existingAdmin = await usersService.findByUsername(
        testUsers.admin.username,
      );
      if (existingAdmin) {
        await usersService.delete(existingAdmin.id);
      }
      const existingUser = await usersService.findByUsername(
  ...
  describe("Authentication Endpoints", () => {
    ...
    describe("POST /auth/api-key", () => {
      ...
      it("should reject API key generation without authentication", async () => {
        await request(app.getHttpServer()).post("/auth/api-key").expect(401);
      });
    });
  });
  ...
  describe("OData Endpoints (API Key Only)", () => {
    describe("GET /odata", () => {
      ...
      it("should reject access without API key", async () => {
        await request(app.getHttpServer()).get("/odata").expect(401);
      });
