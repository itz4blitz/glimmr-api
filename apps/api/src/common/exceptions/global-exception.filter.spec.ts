...
describe("GlobalExceptionFilter", () => {
  ...
  describe("Custom Exception Handling", () => {
    ...
    it("should handle ValidationException", () => {
      ...
      expect(mockResponse.json).toHaveBeenCalledWith({
        statusCode: 422,
        message:
          "Validation failed for field 'email' with value 'invalid-email': must be a valid email",
      ...
      const exception = new HttpException(
        {
          message: [
            ...
            {
              property: "state",
              constraints: { isLength: "state must be 2 characters" },
            },
          ],
          error: "Unprocessable Entity",
          statusCode: 422,
  ...
  describe("HTTP Status Code Mapping", () => {
    const statusCodeMappings = [
      ...
      { status: HttpStatus.NOT_FOUND, expected: ERROR_CODES.NOT_FOUND },
      {
        ...
        expected: ERROR_CODES.METHOD_NOT_ALLOWED,
      },
      {
      ...
      },
      {
        ...
        expected: ERROR_CODES.INTERNAL_SERVER_ERROR,
      },
      {
      ...
      },
      {
        ...
        expected: ERROR_CODES.SERVICE_UNAVAILABLE,
      },
    ];
  ...
  describe("Timestamp Validation", () => {
    ...
    it("should generate different timestamps for different errors", (done) => {
      ...
      const firstTimestamp = (mockResponse.json as jest.Mock).mock.calls[0][0]
        .timestamp;

      // Wait a small amount to ensure different timestamps
      setTimeout(() => {
        filter.catch(exception2, mockArgumentsHost as ArgumentsHost);
        const secondTimestamp = (mockResponse.json as jest.Mock).mock
          .calls[1][0].timestamp;
