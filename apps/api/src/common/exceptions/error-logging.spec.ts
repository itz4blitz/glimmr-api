...
import { HospitalNotFoundException, ExternalServiceException } from './custom-exceptions';
...
describe('Error Logging', () => {
  ...
  describe('Error Details Logging', () => {
    ...
    it('should include custom exception details', () => {
      const exception = new ExternalServiceException('Test API', 'Service unavailable');
  ...
  describe('Log Message Consistency', () => {
    it('should use consistent log message formats', () => {
      const testCases = [
        ...
        {
          exception: new HttpException('Server error', HttpStatus.INTERNAL_SERVER_ERROR),
  ...
  describe('Performance Considerations', () => {
    it('should not log excessively for the same error', () => {
      const exception = new HttpException('Repeated error', HttpStatus.BAD_REQUEST);
    ...
    it('should handle rapid error sequences without performance degradation', () => {
      ...
      for (let i = 0; i < 100; i++) {
        const exception = new HttpException(`Error ${i}`, HttpStatus.BAD_REQUEST);
        filter.catch(exception, mockArgumentsHost as ArgumentsHost);
      }
