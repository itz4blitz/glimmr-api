...
export class ExportDataProcessor extends WorkerHost {
  ...
  async process(job: Job<ExportJobData>): Promise<ExportJobResult> {
    ...
    try {
      ...
      this.updateServiceProgress(exportId, 'processing', 5, 'Initializing export...');

      // Collect data
      await job.updateProgress(10);
      this.updateServiceProgress(exportId, 'processing', 10, 'Collecting data...');

      const exportData = await this.collectExportData(dataset, limit, job);
      
      // Generate file
      await job.updateProgress(70);
      this.updateServiceProgress(exportId, 'processing', 70, 'Generating export file...');

      const fileBuffer = await this.generateExportFile(exportData, format, job);
      
      // Upload to storage
      await job.updateProgress(85);
      this.updateServiceProgress(exportId, 'processing', 85, 'Uploading file...');
      ...
      this.updateServiceProgress(exportId, 'completed', 100, 'Export completed successfully', uploadResult.url);
      ...
      this.updateServiceProgress(exportId, 'failed', 0, `Export failed: ${error.message}`);
  ...
  private async collectExportData(dataset: string, limit: number, job: Job) {
    ...
    if (dataset === 'hospitals' || dataset === 'all') {
      ...
      this.updateServiceProgress(job.data.exportId, 'processing', 20, 'Collecting hospital data...');
      
      data.hospitals = await db.select()
        .from(hospitals)
    ...
    if (dataset === 'prices' || dataset === 'all') {
      ...
      this.updateServiceProgress(job.data.exportId, 'processing', 40, 'Collecting price data...');
      
      data.prices = await db.select()
        .from(prices)
    ...
    if (dataset === 'analytics' || dataset === 'all') {
      ...
      this.updateServiceProgress(job.data.exportId, 'processing', 60, 'Collecting analytics data...');
      
      data.analytics = await db.select()
        .from(analytics)
        .limit(dataset === 'all' ? Math.floor(limit / 3) : limit);
  ...
  private async generateExportFile(data: any, format: string, job: Job): Promise<Buffer> {
    switch (format) {
      case 'json':
  ...
  private generateCSVFile(data: any): Buffer {
    ...
    if (data.hospitals.length > 0) {
      csvContent += 'TYPE,ID,NAME,STATE,CITY,ADDRESS,PHONE,WEBSITE,BED_COUNT,OWNERSHIP\n';
    ...
    if (data.prices.length > 0) {
      ...
      csvContent += 'TYPE,ID,HOSPITAL_ID,DESCRIPTION,CODE,GROSS_CHARGE,DISCOUNTED_CASH_PRICE,CATEGORY\n';
