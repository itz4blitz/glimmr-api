...
import { prices, hospitals, analytics, jobs as jobsTable, jobLogs } from '../../database/schema';
...
export class AnalyticsRefreshProcessor extends WorkerHost {
  ...
  async process(job: Job<AnalyticsRefreshJobData>): Promise<any> {
    ...
    try {
      ...
      await job.updateProgress({ percentage: 5, message: 'Starting analytics calculations' });
        ...
        try {
          ...
          switch (metric) {
            case 'price_statistics':
              recordCount = await this.calculatePriceStatistics(hospitalId, state, period);
              break;
            case 'service_categories':
              recordCount = await this.calculateServiceCategoryMetrics(hospitalId, state, period);
              break;
            case 'payer_rates':
              recordCount = await this.calculatePayerRateMetrics(hospitalId, state, period);
              break;
            ...
            case 'hospital_rankings':
              ...
              break;
            case 'price_trends':
              recordCount = await this.calculatePriceTrends(hospitalId, state, period);
              break;
            case 'data_quality':
              recordCount = await this.calculateDataQualityMetrics(hospitalId, state, period);
              break;
            default:
          ...
          await this.logJobEvent(jobRecord.id, 'error', `Failed to calculate ${metric}`, {
            error: error.message,
          });
          throw error;
        }
      ...
      await job.updateProgress({ percentage: 95, message: 'Cleaning up old analytics' });

      // Clean up old analytics data
      await this.cleanupOldAnalytics(hospitalId, state);

      await job.updateProgress({ percentage: 100, message: 'Analytics refresh completed' });
  ...
  private async calculatePriceStatistics(
    ...
    for (const stat of stats) {
      ...
      const negotiatedCoverage = (stat.pricesWithNegotiatedRates / stat.totalPrices) * 100;
      analyticsRecords.push({
  ...
  private async calculatePayerRateMetrics(
    ...
    // Analyze payer rates
    const payerMetrics: Map<string, { total: number; sum: number; discounts: number[] }> = new Map();
      ...
      try {
        ...
        for (const [payerName, rateInfo] of Object.entries(payerRates)) {
          ...
          if (typeof rate === 'number' && rate > 0 && price.grossCharge) {
            ...
            const discount = grossChargeNum > 0 ? ((grossChargeNum - rate) / grossChargeNum) * 100 : 0;
    ...
    for (const [payerName, metrics] of payerMetrics) {
      ...
      const avgDiscount = metrics.discounts.reduce((a, b) => a + b, 0) / metrics.discounts.length;
  ...
  private async calculateDataQualityMetrics(
    ...
    const qualityStats = await db
      ...
      .groupBy(hospitalId ? sql`1` : hospitals.id, state ? sql`1` : hospitals.state);
    ...
    const analyticsRecords = qualityStats.map(stat => {
      ...
      return {
        ...
        hospitalId: hospitalId || (typeof stat.hospitalId === 'string' ? null : stat.hospitalId),
        ...
        sampleSize: stat.totalPrices,
        metadata: JSON.stringify({
          highQualityPercentage: ((stat.highQuality / stat.totalPrices) * 100).toFixed(2),
          negotiatedRatesCoverage: ((stat.withNegotiatedRates / stat.totalPrices) * 100).toFixed(2),
          cashPriceCoverage: ((stat.withCashPrice / stat.totalPrices) * 100).toFixed(2),
          completeRecordsPercentage: ((stat.completeRecords / stat.totalPrices) * 100).toFixed(2),
        }),
      };
    });
  ...
  private async cleanupOldAnalytics(hospitalId?: string, state?: string): Promise<void> {
    const db = this.databaseService.db;
