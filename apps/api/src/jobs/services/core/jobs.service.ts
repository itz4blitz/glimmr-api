...
export class JobsService {
  ...
  async getJobs(filters: { status?: string; type?: string; limit?: number }) {
    ...
    for (const { name, queue } of allQueues) {
      ...
      try {
        const queueJobs = await this.getJobsFromQueue(
          queue,
          filters.status,
          limit,
        );
      ...
      try {
        ...
        if (completed.length > 0) {
          ...
          const recentJobs = completed
            .slice(0, 100)
            .filter((job) => job.finishedOn && job.finishedOn > oneHourAgo);
  ...
  async startPriceUpdate(updateData: {
    ...
    try {
      ...
      const job = await this.priceUpdateQueue.add("price-update", jobData, {
      ...
      return {
        ...
        estimatedDuration: updateData.hospitalId
          ? "5-15 minutes"
          : "30-90 minutes",
        priority: updateData.priority ?? 5,
        data: jobData,
  ...
  async getDetailedQueueStats() {
          ...
          latestJobs: {
            active: latestActive
              ? {
                  ...
                  startedAt: latestActive.processedOn
                    ? new Date(latestActive.processedOn).toISOString()
                    : null,
                }
              : null,
            completed: latestCompleted
              ? {
                  ...
                  finishedAt: latestCompleted.finishedOn
                    ...
                    : null,
                  duration:
                    latestCompleted.finishedOn && latestCompleted.processedOn
                      ? latestCompleted.finishedOn - latestCompleted.processedOn
                      : null,
                }
              : null,
            failed: latestFailed
              ? {
                  ...
                  failedAt: latestFailed.finishedOn
                    ? new Date(latestFailed.finishedOn).toISOString()
                    : null,
    ...
    return {
      ...
      summary: {
        ...
        totalJobs: detailedStats.reduce(
          (sum, q) =>
            sum +
            q.counts.waiting +
            q.counts.active +
  ...
  async getQueuePerformanceMetrics(queueName: string) {
    ...
    try {
      ...
      for (const [range, cutoff] of Object.entries(timeRanges)) {
        const rangeCompleted = completed.filter(
          (job) => job.finishedOn && job.finishedOn > cutoff,
        );
        const rangeFailed = failed.filter(
          (job) => job.finishedOn && job.finishedOn > cutoff,
        );

        const totalJobs = rangeCompleted.length + rangeFailed.length;
        const successRate =
          totalJobs > 0 ? (rangeCompleted.length / totalJobs) * 100 : 0;
        ...
        metrics[range] = {
          ...
          throughput:
            Math.round(
              (totalJobs / (range === "1h" ? 1 : range === "6h" ? 6 : 24)) *
                100,
            ) / 100, // jobs/hour
        };
      }
    ...
    for (const { name, queue } of allQueues) {
      try {
        ...
        for (let time = cutoffTime; time < now; time += bucketSize) {
          const bucketKey =
            new Date(time).toISOString().substring(0, 13) + ":00:00.000Z"; // Hour precision
        ...
        recentCompleted.forEach((job) => {
          const bucketTime =
            new Date(Math.floor(job.finishedOn / bucketSize) * bucketSize)
              .toISOString()
              .substring(0, 13) + ":00:00.000Z";
        ...
        const trendData = Array.from(buckets.entries()).map(([time, data]) => ({
          ...
          successRate:
            data.completed + data.failed > 0
              ? Math.round(
                  (data.completed / (data.completed + data.failed)) * 10000,
                ) / 100
              : 100,
        }));
          ...
          summary: {
            ...
            avgSuccessRate:
              trendData.length > 0
                ? Math.round(
                    (trendData.reduce(
                      (sum, bucket) => sum + bucket.successRate,
                      0,
                    ) /
                      trendData.length) *
                      100,
  ...
  async getJobLogsById(
    ...
    if (foundJob.progress && typeof foundJob.progress === "object") {
      logs.push({
        ...
        createdAt: new Date(
          foundJob.processedOn || foundJob.timestamp,
        ).toISOString(),
      });
    }
    ...
    try {
      ...
      for (const log of dbLogs) {
        ...
        const logEntry = {
          ...
          level:
            log.level === "info" && log.status === "completed"
              ? "success"
              : log.level,
          message: log.message,
          status: log.status,
  ...
  async getQueueLogs(
    ...
    for (const job of allJobs) {
      ...
      const logLevel = job.failedReason
        ? "error"
        : job.finishedOn
          ? "success"
          : job.processedOn
      ...
      const message = job.failedReason
  ...
  async getJobSystemStatus() {
    ...
    return {
      ...
      systemMetrics: {
        ...
        healthyQueues: queueStatuses.filter((q) => q.health === "healthy")
          .length,
        warningQueues: queueStatuses.filter((q) => q.health === "warning")
          .length,
        criticalQueues: queueStatuses.filter((q) => q.health === "critical")
          .length,
      },
    };
  }
  ...
  private calculateSystemHealth(
    ...
    const warningCount = queueStatuses.filter(
      (q) => q.health === "warning",
    ).length;
  ...
  async searchJobs(filters: any) {
    ...
    const enrichedResults = await Promise.all(
      results.map(async (job) => {
        // Try to find job in queues for real-time data
  ...
  // Bulk retry method
  async bulkRetryJobs(jobIds: string[]) {
    const results = {
      successful: [],
      failed: [],
      notFound: [],
            ...
            if (job) {
              ...
              if (state === "failed") {
                ...
                results.failed.push({
                  jobId,
                  queue: name,
                  reason: `Job is not in failed state (current: ${state})`,
                });
              }
              break;
            }
            ...
            if (job) {
              ...
              if (["waiting", "delayed", "active"].includes(state)) {
                ...
                results.failed.push({
                  jobId,
                  queue: name,
                  reason: `Cannot cancel job in state: ${state}`,
                });
              }
              break;
            }
