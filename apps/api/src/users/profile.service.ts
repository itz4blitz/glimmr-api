...
export class ProfileService {
  ...
  async getUserProfile(userId: string): Promise<{
    ...
    const [profileResult] = await this.db
      .select()
      .from(userProfiles)
      .leftJoin(userPreferences, eq(userProfiles.userId, userPreferences.userId))
  ...
  async createOrUpdateProfile(userId: string, profileData: ProfileUpdateData): Promise<UserProfile> {
    // Check if user exists
    const [user] = await this.db
      .select()
      .from(users)
  ...
  async createOrUpdatePreferences(userId: string, preferencesData: PreferencesUpdateData): Promise<UserPreferences> {
    // Check if user exists
    const [user] = await this.db
      .select()
      .from(users)
  ...
  // File Management
  async uploadFile(userId: string, fileData: FileUploadData): Promise<UserFile> {
    // Validate file
  ...
  private validateFile(fileData: FileUploadData): void {
    ...
    if (fileData.fileType === 'avatar' && !this.allowedImageTypes.includes(fileData.mimeType)) {
      throw new BadRequestException('Invalid file type for avatar. Allowed types: JPEG, PNG, GIF, WebP');
    }
