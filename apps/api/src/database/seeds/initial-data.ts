...
export async function seedInitialData() {
  ...
  try {
    ...
    const hospitalData = await db.insert(hospitals).values([
      {
        name: 'Mayo Clinic',
        state: 'MN',
        city: 'Rochester',
      ...
      priceData.push(
        {
          ...
          serviceCode: '99284',
          codeType: 'CPT',
          description: 'Emergency department visit for evaluation and management of a patient',
          category: 'emergency',
          grossCharge: '2500.00',
    ...
    const insertedPrices = await db.insert(prices).values(priceData).returning();
    console.log(`✅ Seeded ${insertedPrices.length} price records`);

    // Seed analytics
    console.log('📈 Seeding analytics...');
    const analyticsData = await db.insert(analytics).values([
      {
