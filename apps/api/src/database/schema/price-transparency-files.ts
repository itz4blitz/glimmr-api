...
export const priceTransparencyFiles = pgTable(
  ...
  (table) => ({
    ...
    externalFileIdx: index("price_files_external_id_idx").on(
    ...
    hospitalExternalFileIdx: uniqueIndex(
      "price_files_hospital_external_id_idx",
    ).on(table.hospitalId, table.externalFileId),
  }),
);

// Zod schemas for validation
export const insertPriceTransparencyFileSchema = createInsertSchema(
...
export const selectPriceTransparencyFileSchema = createSelectSchema(
  priceTransparencyFiles,
);

export type PriceTransparencyFile = z.infer<
  typeof selectPriceTransparencyFileSchema
>;
export type NewPriceTransparencyFile = z.infer<
  typeof insertPriceTransparencyFileSchema
>;
