...
export const userActivityLogs = pgTable(
  ...
  (table) => ({
    ...
    resourceIdx: index("user_activity_logs_resource_idx").on(
      table.resourceType,
      table.resourceId,
    ),
...
export const userPreferences = pgTable(
  "user_preferences",
  {
    ...
    timezonePreference: varchar("timezone_preference", { length: 50 }).default(
      "UTC",
    ),
  ...
  (table) => ({
    ...
    languageIdx: index("user_preferences_language_idx").on(
...
export type NewPasswordResetToken = z.infer<
