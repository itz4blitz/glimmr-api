...
export const prices = pgTable(
  'prices',
  {
    ...
    discountedCashPrice: decimal('discounted_cash_price', { precision: 12, scale: 2 }),
    minimumNegotiatedCharge: decimal('minimum_negotiated_charge', { precision: 12, scale: 2 }),
    maximumNegotiatedCharge: decimal('maximum_negotiated_charge', { precision: 12, scale: 2 }),

    // Legacy fields for backward compatibility
    minimumNegotiatedRate: decimal('minimum_negotiated_rate', { precision: 12, scale: 2 }),
    maximumNegotiatedRate: decimal('maximum_negotiated_rate', { precision: 12, scale: 2 }),
    ...
    dataSource: varchar('data_source', { length: 100 }).default('price_transparency_file'),
  ...
  (table) => [
    ...
    index('prices_hospital_service_idx').on(table.hospitalId, table.serviceName),
...
export const insertPriceSchema = z.object({
  ...
  discountedCashPrice: z.string().regex(/^\d+(\.\d{1,2})?$/).optional(),
  minimumNegotiatedRate: z.string().regex(/^\d+(\.\d{1,2})?$/).optional(),
  maximumNegotiatedRate: z.string().regex(/^\d+(\.\d{1,2})?$/).optional(),
