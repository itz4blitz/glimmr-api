...
describe("AnalyticsController", () => {
  ...
  describe("getDashboardAnalytics", () => {
    it("should return dashboard analytics successfully", async () => {
      const mockResult = {
        ...
        priceVariation: 0.75,
        topServices: [
          { service: "MRI", count: 1500, averagePrice: 1800.0 },
          { service: "CT Scan", count: 1200, averagePrice: 1200.0 },
        ],
        stateBreakdown: {
  ...
  describe("getPricingTrends", () => {
    ...
    it("should throw SERVICE_UNAVAILABLE when database connection fails", async () => {
      ...
      await expect(controller.getPricingTrends({})).rejects.toThrow(
        HttpException,
      );
  ...
  describe("Rate Limiting Behavior Simulation", () => {
    ...
    it("should generate different keys for different users", () => {
      // Anonymous user
      const key1 = throttlerGuard["generateKey"](
        mockContext,
        "suffix",
        "expensive",
      );
      ...
      const key2 = throttlerGuard["generateKey"](
        mockContext,
        "suffix",
        "expensive",
      );
    ...
    it("should handle X-Forwarded-For header for proxy scenarios", () => {
      ...
      const clientId = throttlerGuard["getClientId"](
  ...
  describe("getPricingTrends", () => {
    ...
    it("should handle different period values", async () => {
      ...
      for (const period of periods) {
        ...
        const expectedResult = {
          period,
          trends: [{ date: "2024-01-01", averagePrice: 750 }],
          summary: {
            startPrice: 750,
            endPrice: 750,
            changePercent: 0,
  ...
  describe("getPowerBIInfo", () => {
    ...
    it("should call analyticsService.getPowerBIInfo", async () => {
      const expectedResult = {
        ...
        dashboardUrl:
          "https://app.powerbi.com/groups/workspace-456/dashboards/dashboard-789",
  ...
  describe("downloadExportData", () => {
    ...
    it("should stream export data successfully with default parameters", async () => {
      ...
      await controller.downloadExportData(
        query,
    ...
    it("should throw SERVICE_UNAVAILABLE when database connection fails during streaming", async () => {
      const query = { format: "json", dataset: "hospitals" };
      const connectionError = new Error("ECONNREFUSED");
      mockAnalyticsService.downloadExportData.mockRejectedValue(
        connectionError,
      );
