...
export class PricesController {
  ...
  async getPricingAnalytics(
    ...
    try {
      ...
      throw new HttpException(
        {
          message:
            "Internal server error occurred while fetching pricing analytics",
          statusCode: HttpStatus.INTERNAL_SERVER_ERROR,
          error: "Internal Server Error",
        },
        HttpStatus.INTERNAL_SERVER_ERROR,
  ...
  @ApiQuery({
    name: "zipcode",
    required: true,
    description: "Zipcode to search around",
  })
  @ApiQuery({
    name: "radius",
    required: false,
  ...
  @ApiQuery({
    ...
    required: false,
    description: "Include only prices with negotiated rates",
  })
  @ApiQuery({
    name: "limit",
    required: false,
    ...
    try {
      ...
      return await this.pricesService.searchPricesByZipcode({
        ...
        includeNegotiatedRates:
          includeNegotiatedRates === "true" || includeNegotiatedRates === true,
  ...
  @ApiResponse({
    status: 200,
    description: "Hospital prices retrieved successfully",
  })
  @ApiResponse({
    status: 401,
    description: "Unauthorized - Authentication required",
  ...
  @ApiQuery({
    name: "codeType",
    required: false,
    description: "Filter by code type (CPT, DRG, HCPCS, etc.)",
  })
  @ApiQuery({
    name: "limit",
    required: false,
  ...
  async getHospitalPrices(
    ...
    try {
      ...
      throw new HttpException(
        {
          message:
            "Internal server error occurred while fetching hospital prices",
          statusCode: HttpStatus.INTERNAL_SERVER_ERROR,
          error: "Internal Server Error",
        },
        HttpStatus.INTERNAL_SERVER_ERROR,
