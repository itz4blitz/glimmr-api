...
export class SessionActivityDto {
  @ApiProperty({ description: 'Session action type', enum: ['start', 'ping', 'end'] })
  @IsString()
  @IsIn(['start', 'ping', 'end'])
...
export class ActivityController {
  ...
  async getActivityStats(
    ...
    const stats = await this.activityLoggingService.getActivityStats(timeRange || '24h');
    return stats;
  }
  ...
  private convertToCSV(activities: any[]): string {
    ...
    const headers = ['Timestamp', 'User ID', 'User Email', 'Action', 'Resource Type', 'Resource ID', 'Success', 'IP Address', 'User Agent'];
