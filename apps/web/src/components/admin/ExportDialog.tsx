        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-4">
              {fieldCategories.map((category) => {
                const categoryFields = availableFields.filter(field => field.category === category.key)
                const selectedInCategory = categoryFields.filter(field => selectedFields.includes(field.key))
                    ...
                    <div className="flex items-center gap-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => 
                          handleSelectAllInCategory(category.key, checked as boolean)
                        }
          ...
          <div className="space-y-2">
            ...
            <div className="text-sm text-muted-foreground space-y-1">
              ...
              <div>Filters: {Object.values(filters).filter(v => v && v !== 'all').length > 0 ? 'Applied' : 'None'}</div>
            </div>
          </div>
        </div>
