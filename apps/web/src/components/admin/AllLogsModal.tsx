...
export function AllLogsModal({ isOpen, onClose }: AllLogsModalProps) {
  ...
  }, [isOpen, levelFilter, queueFilter, statusFilter, searchTerm, isAutoRefresh])
    ...
    return filteredLogs
      .filter(log => {
        const isActive = log.status === 'active' || 
                        ...
                        (log.status === 'unknown' && log.level === 'info' && log.message?.toLowerCase().includes('job'))
        return isActive
      })
      .map(log => {
  ...
  const exportLogs = () => {
    const csvContent = [
      ['Timestamp', 'Queue', 'Job ID', 'Level', 'Status', 'Message', 'Duration'].join(','),
      ...
      <Dialog open={isOpen} onOpenChange={onClose}>
        ...
        {(() => {
          const stuckJobs = filteredLogs.filter(log => {
            if (log.status !== 'active' && !log.message?.toLowerCase().includes('is being processed')) return false
            const elapsed = Date.now() - new Date(log.timestamp || log.createdAt).getTime()
              ...
              <div className="bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800 rounded-lg p-3 mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-sm font-medium text-red-600">
                    {stuckJobs.length} job{stuckJobs.length > 1 ? 's' : ''} may be stuck
                  </p>
        ...
        <div className="flex flex-wrap gap-3 py-4 border-b">
          ...
          <Select value={queueFilter} onValueChange={setQueueFilter}>
            ...
            <SelectContent>
              ...
              <SelectItem value="price-file-download">Price File Parser</SelectItem>
              <SelectItem value="price-update">Price Normalization</SelectItem>
              <SelectItem value="analytics-refresh">Analytics Refresh</SelectItem>
              <SelectItem value="export-data">Data Export</SelectItem>
              <SelectItem value="pra-unified-scan">Hospital Discovery</SelectItem>
              <SelectItem value="pra-file-download">File Downloads</SelectItem>
            </SelectContent>
          </Select>
          ...
          {isLoading && filteredLogs.length === 0 ? (
            ...
            <div className="flex flex-col items-center justify-center h-64 text-center">
              ...
              <p className="text-muted-foreground">
                {searchTerm || levelFilter !== 'all' || queueFilter !== 'all' || statusFilter !== 'all'
                                    ...
                                    <div>
                                      ...
                                      <dl className="space-y-2 text-sm">
                                        ...
                                        <div className="flex justify-between py-1 border-b border-border/30">
                                          ...
                                          <dd className="font-mono text-xs text-right break-all" title={log.jobId}>
                                            {log.jobId}
                                          </dd>
                                        </div>
                                        <div className="flex justify-between py-1 border-b border-border/30">
                                          <dt className="font-medium text-muted-foreground">Log ID:</dt>
                                          <dd className="font-mono text-xs text-right">{log.id}</dd>
                                        </div>
                                        <div className="flex justify-between py-1 border-b border-border/30">
                                          <dt className="font-medium text-muted-foreground">Queue:</dt>
                                          <dd className="text-right">{log.queueName}</dd>
                                        </div>
        ...
        <div className="flex justify-between items-center pt-4 border-t">
          <div className="text-sm text-muted-foreground">
            ...
            {isAutoRefresh && <span className="ml-2">(Auto-refreshing every 10s)</span>}
          </div>
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="space-y-2">
              <span className="block">
                This action will permanently delete all job logs from the database. 
