...
import { AlertTriangle, AlertCircle, Clock, RefreshCw, XCircle } from 'lucide-react'
...
export function JobHealthIndicator({
  ...
  const getHealthStatus = () => {
    if (jobName?.includes('download') || message?.toLowerCase().includes('download')) {
      // File download jobs
      ...
      return 'healthy'
    } else if (jobName?.includes('parse') || message?.toLowerCase().includes('parse')) {
      // Parsing jobs
  ...
  const healthConfig = {
    critical: {
      ...
      description: 'This job has been running much longer than expected. It may be stuck or experiencing issues.'
    },
    warning: {
      icon: AlertTriangle,
      ...
      description: 'This job is taking longer than typical. Monitor for potential issues.'
    },
    caution: {
      icon: AlertCircle,
      ...
      description: 'This job is taking a while. This might be normal for large files.'
    },
    healthy: {
      icon: Clock,
      color: 'text-blue-600',
  ...
  return (
    <div className={cn(
      ...
      {progress && (progress.percentage !== undefined || progress.bytesDownloaded !== undefined) && (
