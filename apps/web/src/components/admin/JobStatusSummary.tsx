...
export function JobStatusSummary({ log, elapsedTime }: JobStatusSummaryProps) {
  ...
  const getJobInfo = () => {
    ...
    if (queueName.includes('download') || message.includes('download')) {
      ...
      else if (message.includes('starting download')) action = 'Starting download'
      else if (message.includes('downloaded')) action = 'Download progress'
      else if (message.includes('updating database')) action = 'Saving metadata'
    ...
    } else if (queueName.includes('analytics') || message.includes('analytics')) {
      type = 'analytics'
      icon = BarChart
      action = 'Calculating'
    ...
    const extractInfo = () => {
      ...
      // Extract file name
      const fileMatch = message.match(/(?:file|downloading|parsing)\s+([^\s]+\.(csv|json|xml|txt))/i)
      if (fileMatch) info.push(`File: ${fileMatch[1]}`)
      
      // Extract hospital ID
      const hospitalMatch = message.match(/hospital[:\s]+([a-f0-9-]+)/i)
      if (hospitalMatch) info.push(`Hospital: ${hospitalMatch[1].substring(0, 8)}...`)
  ...
  const getStatusIcon = () => {
    ...
    } else if (log.status === 'active' || log.message.includes('processing') || log.message.includes('started')) {
  ...
  // Get progress info
  const progress = log.context?.progress || log.context
  const hasProgress = progress?.percentage !== undefined || progress?.bytesDownloaded !== undefined
        ...
        {jobInfo.info.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {jobInfo.info.map((info, i) => (
              <span key={i} className="text-xs bg-background px-2 py-0.5 rounded-md border">
                {info}
              </span>
            ))}
          </div>
            ...
            {progress.percentage !== undefined && (
              <div className="space-y-1">
                <div className="flex justify-between text-xs">
                  ...
                  <span className="font-mono font-medium">{progress.percentage.toFixed(1)}%</span>
                </div>
                <Progress value={progress.percentage} className="h-1.5" />
              </div>
            )}
            
            {(progress.bytesDownloaded !== undefined || progress.speed !== undefined) && (
              <div className="flex gap-4 text-xs">
                {progress.bytesDownloaded !== undefined && (
                  <span className="text-muted-foreground">
                    Downloaded: <span className="font-mono text-foreground">
                ...
                {progress.speed !== undefined && progress.speed > 0 && (
                  <span className="text-muted-foreground">
                    Speed: <span className="font-mono text-foreground">
                ...
                {progress.eta && (
                  <span className="text-muted-foreground">
                    ETA: <span className="font-mono text-foreground">{progress.eta}</span>
                  </span>
                )}
              </div>
            )}
