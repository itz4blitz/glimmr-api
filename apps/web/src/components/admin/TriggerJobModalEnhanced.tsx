...
interface TriggerJobModalEnhancedProps {
...
export function TriggerJobModalEnhanced({ queueName, isOpen, onClose, onSuccess }: TriggerJobModalEnhancedProps) {
  ...
  const [selectedStates, setSelectedStates] = useState<string[]>(['CA', 'FL', 'TX'])
  ...
  useEffect(() => {
    if (isOpen && (queueName === 'pra-file-download' || queueName === 'price-file-parser')) {
      fetchHospitals()
    }
  }, [isOpen, queueName])
  ...
  const fetchPriceFiles = async (hospitalId: string) => {
    try {
      const response = await apiClient.get(`/hospitals/${hospitalId}/price-files`)
    ...
    switch (queueName) {
      ...
      case 'pra-unified-scan': {
        return {
          ...
          states: selectedStates.length === ALL_STATES.length ? undefined : selectedStates, // undefined means all states
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
        ...
        <Tabs value={mode} onValueChange={(v) => handleModeChange(v as 'guided' | 'manual')}>
          ...
          <TabsContent value="guided" className="space-y-4">
            {(queueName === 'pra-file-download' || queueName === 'price-file-parser' || queueName === 'price-update') && (
              <>
                <div className="space-y-2">
                  ...
                  <Select value={selectedHospital} onValueChange={setSelectedHospital}>
                    <SelectTrigger>
                      <SelectValue placeholder={isLoadingData ? "Loading hospitals..." : "Choose a hospital"} />
                    </SelectTrigger>
                    <SelectContent>
                      {filteredHospitals.map(hospital => (
                        <SelectItem key={hospital.id} value={hospital.id}>
                          <div className="flex items-center gap-2">
                            ...
                            <span>{hospital.name} - {hospital.city}, {hospital.state}</span>
                          </div>
                        </SelectItem>
                      ))}
                    </SelectContent>
                ...
                {(queueName === 'pra-file-download' || queueName === 'price-file-parser') && selectedHospital && (
                  <div className="space-y-2">
                    <Label htmlFor="file">Select Price File</Label>
                    <Select value={selectedFile} onValueChange={setSelectedFile}>
                      <SelectTrigger>
                        <SelectValue placeholder={priceFiles.length === 0 ? "No files found" : "Choose a file"} />
                      </SelectTrigger>
                      <SelectContent>
                        {priceFiles.map(file => (
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <div className="flex flex-col">
                                ...
                                <span className="text-xs text-muted-foreground">
                                  {file.fileType.toUpperCase()} • {file.fileSize ? `${(file.fileSize / 1024 / 1024).toFixed(2)} MB` : 'Unknown size'} • {file.processingStatus}
                                </span>
                              </div>
                            </div>
                          </SelectItem>
            ...
            {queueName === 'pra-unified-scan' && (
              <div className="space-y-2">
                ...
                <div className="border rounded-lg p-4 max-h-64 overflow-y-auto">
                  <div className="grid grid-cols-5 gap-2">
                    {ALL_STATES.map(state => (
                      <label key={state} className="flex items-center gap-1.5 cursor-pointer hover:bg-secondary/50 rounded px-2 py-1">
                        <input
                          type="checkbox"
                ...
                <p className="text-xs text-muted-foreground">
                  ...
                  {selectedStates.length === ALL_STATES.length && ' (All states)'}
                </p>
                ...
                <div className="flex items-center gap-2 mt-4">
                  ...
                  />
                  <Label htmlFor="forceRefresh" className="cursor-pointer font-normal">
                    Force refresh (ignore last check time and re-scan all hospitals)
                  </Label>
                </div>
              </div>
            )}
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-muted-foreground">
                This data will be sent to the job queue. In guided mode, it's automatically generated based on your selections.
              </p>
            </div>
          </TabsContent>
          ...
          <TabsContent value="manual" className="space-y-4">
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-muted-foreground">
                Enter the job data as valid JSON. You can switch to Guided Mode for help with the structure.
              </p>
            </div>
          </TabsContent>
        </Tabs>
