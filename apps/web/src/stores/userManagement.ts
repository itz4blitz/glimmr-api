  ...
  devtools(
    (set, get) => ({
      ...
      updateUserProfile: async (id, data) => {
        try {
          const updatedUser = await userManagementApi.updateUserProfile(
            id,
            data,
          );
      ...
      updateUserPreferences: async (id, data) => {
        try {
          const updatedUser = await userManagementApi.updateUserPreferences(
            id,
            data,
          );
      ...
      bulkAction: async (action) => {
        ...
        try {
          ...
          if (result.failed.length > 0) {
            toast.warning(
              `${result.successful.length} users processed, ${result.failed.length} failed`,
            );
          } else {
            toast.success(
